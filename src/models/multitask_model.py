...
class MultiTaskModel(nn.Module):
    """共享编码器 + 分割头 + 分类头。"""
    def __init__(self, backbone_name: str='resnet18', num_classes: int=1, seg_upsample_to_input: bool=True):
        super().__init__()
        self.encoder = get_backbone(backbone_name)
        in_ch = self.encoder.out_channels
        self.seg_head = SimpleSegHead(in_channels=in_ch, upsample_to_input=seg_upsample_to_input)
