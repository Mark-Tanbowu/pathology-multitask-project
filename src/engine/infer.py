...
def main():
    ...
    ap.add_argument('--image', type=str, default=None, help='单张patch图像路径（PNG/JPG）')
    ap.add_argument('--ckpt', type=str, default=None, help='模型权重（best.pt），可为空使用随机权重')
    ...
    print(f"Done. cls_prob={cls_prob:.3f} | mask_out={args.mask_out} | overlay={args.overlay_out}")
