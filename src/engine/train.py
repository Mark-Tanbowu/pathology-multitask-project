...
def main(cfg):
    ...
    # Model & Loss & Optim
    model = MultiTaskModel(backbone_name=cfg.model.backbone, num_classes=cfg.model.num_classes,
                           seg_upsample_to_input=cfg.model.seg_upsample_to_input).to(device)
    criterion = MultiTaskLoss(seg_type=cfg.loss.seg, cls_type=cfg.loss.cls,
                              seg_weight=cfg.loss.seg_weight, cls_weight=cfg.loss.cls_weight)
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        print(f"Epoch {epoch:03d}: train_loss={train_loss:.4f} | val_loss={val_loss:.4f} | val_dice={val_dice:.4f} | val_acc={val_acc:.4f}")
