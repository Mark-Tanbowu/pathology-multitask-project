...
class MultiTaskLoss(nn.Module):
    def __init__(self, seg_weight: float = 1.0, cls_weight: float = 1.0, dice_weight: float = 0.5):
        super().__init__()
    ...
    def forward(self, outputs, batch):
        ...
        if outputs.get("seg") is not None and batch.get("mask") is not None:
            ...
            loss = loss + self.seg_weight * (self.dice_weight * dice + (1 - self.dice_weight) * bce)
