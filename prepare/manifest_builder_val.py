...
def infer_camelyon_slide_label(slide_id: str) -> int | None:
    """Infer CAMELYON slide label from slide_id prefix."""
    sid = slide_id.lower()
...
def select_stratified_random(
    ...
    """Sample patches per slide with class stratification."""
...
def build_manifest(
    ...
    apply_xml_rules_for_tumor_only: bool,
) -> None:
    ...
    if slide_label_map:
        print(f"[INFO] loaded {len(slide_label_map)} slide labels from {slide_labels_csv}")
    ...
    try:
        ...
        for slide_idx, slide_path in enumerate(slides, start=1):
            ...
            print(f"[INFO] slide {slide_idx}/{len(slides)}: {slide_id}")
            gt_slide_label = slide_label_map.get(slide_id)
            if gt_slide_label is None:
                gt_slide_label = infer_camelyon_slide_label(slide_id)
            is_tumor_slide = int(gt_slide_label or 0) > 0

            xml_path = None
            if annotations_dir is not None:
                allow_xml = is_tumor_slide or (not apply_xml_rules_for_tumor_only)
                if allow_xml:
                    ...
                    if candidate.exists():
                        ...
                        raise FileNotFoundError(
                            f"Missing annotation XML for positive slide: {slide_id} -> {candidate}"
                        )
            ...
            with SlideReader(slide_path) as slide:
                ...
                ignore_polygons = (
                    ...
                    if (xml_path and is_tumor_slide and ignore_groups)
                    else []
                )
                mask = None
                ...
                # train/val 保持原策略：若配置了 annotations_dir，则标注优先走 XML。
                ...
                if enable_geometry_stats and is_tumor_slide:
                ...
                for y in range(0, height - patch_size + 1, stride):
                    for x in range(0, width - patch_size + 1, stride):
                        ...
                        # 忽略区仅对 tumor slide 启用，normal 保持原有流程。
                        if is_tumor_slide and ignore_polygons:
            ...
            if gt_slide_label is None:
                if slide_label_fallback_to_sampled:
                    locked_slide_label = sampled_has_pos
                else:
                    raise ValueError(
                        f"Cannot resolve ground-truth slide_label for slide_id={slide_id}. "
                        "Set slide_labels_csv or enable slide_label_fallback_to_sampled."
                    )
            else:
                locked_slide_label = int(gt_slide_label)

            if sampled_has_pos != locked_slide_label:
                total_sampled_gt_mismatch += 1
                print(
                    f"[WARN] sampled_has_pos={sampled_has_pos} differs from locked slide_label="
            ...
            print(
                f"[INFO] done slide={slide_id} patches={patches_written} "
                f"excluded={excluded_by_ignore} elapsed={slide_elapsed:.1f}s "
                f"total={total} pos={total_pos} neg={total_neg}"
            )
...
def build_manifest_from_config(config: dict) -> None:
    ...
    apply_xml_rules_for_tumor_only = bool(data_cfg.get("apply_xml_rules_for_tumor_only", True))
    coords_out_dir = data_cfg.get("coords_out_dir", "data/processed/coords/val")
    ...
    build_manifest(
        ...
        apply_xml_rules_for_tumor_only=apply_xml_rules_for_tumor_only,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--slide-manifest-path",
        type=Path,
        default=Path("data/processed/manifest_slides_val.csv"),
    )
    ...
    parser.add_argument("--disable-tumor-only-xml-rules", action="store_true")
...
def main() -> None:
    ...
    if args.config is not None:
        ...
        prepare_cfg = config.get("prepare_val") or config.get("prepare")
        if prepare_cfg is None:
            raise ValueError("Missing prepare_val (or prepare) section in config.")
    ...
    build_manifest(
        ...
        apply_xml_rules_for_tumor_only=not args.disable_tumor_only_xml_rules,
    )
