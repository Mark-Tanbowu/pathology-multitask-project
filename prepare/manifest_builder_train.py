...
def build_manifest_from_config(config: dict) -> None:
    ...
    coords_out_dir = data_cfg.get("coords_out_dir", "data/processed/coords/train")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--slide-manifest-path",
        type=Path,
        default=Path("data/processed/manifest_slides_train.csv"),
    )
...
def main() -> None:
    ...
    if args.config is not None:
        ...
        prepare_cfg = config.get("prepare_train")
        if prepare_cfg is None:
            raise ValueError("缺少 defaults.yaml 中的 prepare_train 配置。")
